    ...
    if ($_POST) {
        if (!isset($_POST['data_agendamento_consulta_inicio'])) {
            if (!isset($_POST['local_origem'])) {
                $array_filter = $object->readQuiosque();
            } else {
                if ($_POST['local_origem'] == 0) {
                    echo '<script>alert("Precisa selecionar pelo menos uma quadra e um horário!");</script>';
                    $array_filter = $object->readQuiosque();
                } else {
            ...
            $array_filter = $object->readQuiosqueFilter($_POST['local_origem_consulta'], $_POST['data_agendamento_consulta_inicio'], $_POST['data_agendamento_consulta_fim']);
        }
    } else {
        $array_filter = $object->readQuiosque();
    }
...
?>
<div class="h2 text-center mb-3 fw-bold">Controle de Quiosques</div>
